...
class EnhancedRAGStore(RAGStore):
    """
    Enhanced RAG Store with multi-memory architecture support.
    Supports specific datasets: agent_core, context_flow, agent_memory, projects.
    """
    
    VALID_DATASETS = {
        "agent_core",    # Permanent rules
        "context_flow",  # Pre/Post summaries
        "agent_memory",  # Feedbacks, lessons
        "projects",      # Code & analytical docs
        "scratchpad"     # Temporary (legacy support)
    }

    async def auto_route(self, dataset_choice: str) -> str:
        """
        Route generic dataset choices to specific memory stores.
        
        Args:
            dataset_choice: User or system provided dataset name
            
        Returns:
            Canonical dataset name
        """
        if dataset_choice in self.VALID_DATASETS:
            return dataset_choice
            
        # Mapping logic for aliases
        mapping = {
            "core": "agent_core",
            "rules": "agent_core",
            "memory": "agent_memory",
    ...
    def parse_metadata(self, metadata: Dict[str, Any]) -> Dict[str, Any]:
        """
        Enforce metadata structure for enhanced memory.
    ...
    async def add_document(
        ...
        """
        Add document with enhanced routing and metadata validation.
        
        Args:
            dataset: Target dataset (or alias)
        ...
        Returns:
            Document ID
        """
        # Auto-route dataset if needed
        target_dataset = await self.auto_route(dataset)
    ...
    async def query(
        ...
        """
        target_dataset = await self.auto_route(dataset)
        return await super().query(target_dataset, question, top_k)
